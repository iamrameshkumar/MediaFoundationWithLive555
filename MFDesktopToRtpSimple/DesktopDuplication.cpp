...
bool cImageCapturingModuleId3d11Impl::GetCurrentFrameAsVideoSample(void **videoSample, bool &isTimeout, int surfaceWidth, int surfaceHeight) {
	...
	if (pNewTexture) {
		...
		MFCreateDXGISurfaceBuffer(__uuidof(ID3D11Texture2D), pNewTexture, 0, FALSE, &pMediaBuffer);

		CComPtr<IMF2DBuffer> p2DBuffer;
		DWORD length;
		pMediaBuffer->QueryInterface(__uuidof(IMF2DBuffer), reinterpret_cast<void **>(&p2DBuffer));
		p2DBuffer->GetContiguousLength(&length);
		pMediaBuffer->SetCurrentLength(length);

		//MFCreateVideoSampleFromSurface(NULL, (IMFSample**)videoSample);
		MFCreateSample((IMFSample**)videoSample);
		(*((IMFSample**)videoSample))->AddBuffer(pMediaBuffer);
...
SmartPtr<BitmapData>* cImageCapturingModuleId3d11Impl::GetCurrentFrameImage(CRect &rect)
{
	...
	TRY
	{
		...
		{
			D3D11_TEXTURE2D_DESC textureDesc, requiredTextureDesc;
			m_CurrentFrameTexture->GetDesc(&textureDesc);
			m_CurrentFrameTexture->GetDesc(&requiredTextureDesc);
